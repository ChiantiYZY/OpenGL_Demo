...
void main()
{
    ...
    vec4 H = normalize((fs_CameraPos + fs_LightVec) / 2);
//vec4 H = normalize(fs_LightVec);

    float exp = 2.0f;

    float specular = max(pow(dot(H, normalize(fs_Nor)), exp), 0.0f);

    float lightIntensity = diffuseTerm + ambientTerm;   //Add a small float value to the color multiplier
                                                        //to simulate ambient lighting. This ensures that faces that are not                                                       //lit by our point light are not completely black.

    // Compute final shaded color
    out_Col = vec3(diffuseColor.rgb * lightIntensity) + vec3(1,1,1) * specular;
